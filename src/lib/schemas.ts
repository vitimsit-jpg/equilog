...
export const ShareCreateSchema = z.object({
  email: z
    ...
    .toLowerCase(),
  role: z.enum(["gerant", "coach"], {
    errorMap: () => ({ message: "Rôle invalide (gerant ou coach)" }),
  }),
...
export const ReactionSchema = z.object({
  item_type: z.string({ required_error: "item_type requis" }).min(1),
  item_id: z.string({ required_error: "item_id requis" }).min(1),
  reaction_type: z.enum(["like", "fire", "strong", "trophy"]).default("like"),
});
export type Reaction = z.infer<typeof ReactionSchema>;

export const CoachChatSchema = z.object({
  horseId: z.string({ required_error: "horseId requis" }).uuid("horseId invalide"),
  messages: z
    .array(
      z.object({
...
export const VoiceTranscribeSchema = z.object({
  transcript: z.string({ required_error: "transcript requis" }).min(1),
